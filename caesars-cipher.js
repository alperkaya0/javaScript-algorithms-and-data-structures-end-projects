function rot13(str) {
  ...
  for (let i = 0; i < str.length; ++i) {
    if (/[\w]/.test(str[i])) {//check if the char consists of alphanumeric elements
      ...
      if (idx < 0) {//you may think this as we are making alp array like a linked list that shapes a circle (the element after the last element is the first element)
        idx += alp.length;
      }
